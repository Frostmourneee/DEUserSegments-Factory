...
def check_data_quality():
    ...
    try:
        ...
        if latest_calc != today:
            raise ValueError(
                f"User segments are not up to date! Latest: {latest_calc}, Expected: {today}"
            )
        ...
        if latest_mart != today:
            raise ValueError(
                f"Marketing mart is not up to date! Latest: {latest_mart}, Expected: {today}"
            )
        ...
        if null_records > 0:
            raise ValueError(
                f"Found {null_records} records with NULL values in marketing mart!"
            )
